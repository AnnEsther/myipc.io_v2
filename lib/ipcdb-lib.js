...
{
    console.log("IPCDB_USERNAME : "+ config.IPCDB_USERNAME);
    console.log("IPCDB_PASSWORD : "+ config.IPCDB_PASSWORD);
    console.log("IPCDB_DATABASE : "+ config.IPCDB_DATABASE);
    console.log("IPCDB_HOST : "+ config.IPCDB_HOST);
...
async function ipcdb_restore_database(database)
{
    ...
    for (; index < database.length; index++)
    {
        ...
        ipc.meta.sprite = ipc.token_id;//await IPCGif.ipcgif_store(ipc);
        ipc.meta.card = ipc.token_id;//await IPCCard.ipccard_store(ipc);
